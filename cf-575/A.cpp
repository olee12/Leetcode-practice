...
long long splitt(long long alice, long long bob, long long ext) {
    if (alice < bob) {
        ll less = bob - alice;
        alice += min(less, ext);
        ext -= min(less,ext);
...
int main()
{
    ...
    while(test--) {
        ...
        ll ret = 0;
        ret = splitt(a,b,c);
        ret = max(ret,splitt(a,b,c));
        ret = max(ret,splitt(a,c,b));
        ret = max(ret,splitt(b,a,c));
        ret = max(ret,splitt(b,c,a));
        ret = max(ret,splitt(c,a,b));
        ret = max(ret,splitt(c,b,a));
        pf("%lld\n",ret);
    }
    return 0;
}
