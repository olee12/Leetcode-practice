...
template <typename T>
inline T pmod(T n, T m) { return ((n % m) + m) % m; }
template <typename T>
T power(T n, int p) { return (p == 0) ? (T)(1) : (sqr(power(n, p >> 1)) * ((p & 1) * (n - 1) + 1)); }
template <typename T>
T bigmod(T n, int p, T m) { return (p == 0) ? (T)(1) : ((sqr(bigmod(n, p >> 1, m)) % m) * ((p & 1) * (n - 1) % m + 1)) % m; }
...
int rec(string pattern,int n,int k) {
    int ret = maxn;
    int miss = 0;
    for(int i = 1;i<=k;i++){
        if(pattern[i%3] != inp[i]) miss++;
    }


    ret = min(ret,miss);
    // cout<<pattern<<' '<<inp<<' '<<ret<<' '<<miss<<endl;
    for(int i = k+1;i<=n;i++){
        if (pattern[(i-k) % 3] != inp[i-k]) miss--;
        if(pattern[i%3] != inp[i]) miss++;
        ret = min(ret,miss);
    }
    return ret;
...
int main() {
    ...
    while(test--) {
        sf2(n,k);
        sf("%s",inp+1);
        int ret = min(rec("RGB",n,k), min(rec("GBR",n,k),rec("BRG",n,k)));
        printf("%d\n",ret);
    }
